...
source = (ROOT/'docs/whitepaper/Nexa公众白皮书_v0.2.md').read_text()
pdf = ROOT/'output/pdf/Nexa公众白皮书_v0.2.pdf'
...
blocks = [public_block(block) for block in source.split('## 01', 1)[1].split('\n\n') if block.strip()]
assert 9 <= len(reader.pages) <= 15, 'PDF 页数不符'
assert all(normalize(block) in pdf_text for block in blocks), 'PDF 缺少文稿内容'
page = Page()
page.feed((ROOT/'dist/whitepaper/index.html').read_text())
assert page.h1 == 1 and page.h2 == 7, '网页标题结构错误'
downloads = dict(page.downloads)
expected_downloads = {
    'Nexa公众白皮书_v0.1.pdf': ROOT/'output/pdf/Nexa公众白皮书_v0.1.pdf',
    'Nexa公众白皮书_v0.2.pdf': pdf,
}
...
print(f'通过：{len(reader.pages)} 页 PDF、{len(blocks)} 个正文块三端一致、七章目录、单一 H1、v0.1/v0.2 下载一致、基础敏感模式扫描。')
print(f'PDF 大小 {pdf.stat().st_size} 字节；SHA256 {hashlib.sha256(pdf.read_bytes()).hexdigest()}')
