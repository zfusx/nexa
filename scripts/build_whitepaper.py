"""从公开 Markdown 生成专业版审阅 PDF；绝不读取 white_paper/ 内部资料。
...
SOURCE = ROOT / 'docs/whitepaper/Nexa公众白皮书_v0.2.md'
OUTPUT = ROOT / 'output/pdf/Nexa公众白皮书_v0.2.pdf'
...
def cover_page(canvas, doc):
    ...
    canvas.setFillColor(colors.HexColor('#b9aceb'))
    canvas.drawString(56, HEIGHT-340, '传统知识 · 数学建模 · 概率与稳定性 · AI 解释')
    ...
    draw_paragraph(canvas, cover.strip().split('\n\n')[-1], 56, 232, 455, note)
...
def body_page(canvas, doc):
    ...
    canvas.drawString(54, 36, '传统知识 · 数学建模 · 概率与稳定性 · AI 解释')
...
doc = SimpleDocTemplate(str(OUTPUT), pagesize=A4, leftMargin=54, rightMargin=54, topMargin=84, bottomMargin=76, title='Nexa 方法与数学框架白皮书 v0.2', author='Nexa', subject='数学框架与证据治理 · 专业版审阅稿')
doc.build(story, onFirstPage=cover_page, onLaterPages=body_page)
assert 9 <= doc.page <= 15, f'排版页数异常，预期 9-15 页，实际 {doc.page} 页'
